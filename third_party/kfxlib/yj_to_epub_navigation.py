...
class KFX_EPUB_Navigation(object):
    ...
    def process_nav_unit(self, nav_type, nav_unit, ncx_toc, nav_container_name, section_name, heading_level):
        ...
        if "$246" in nav_unit:
            ...
            target_position = self.get_position(nav_unit.pop("$246"))
            self.register_anchor(anchor_name, target_position, heading_level)
        else:
            anchor_name = None
    ...
    def register_anchor(self, anchor_name, position, heading_level=None):
        ...
        if heading_level is not None:
            self.anchor_heading_level[anchor_name] = heading_level
    ...
    def fixup_anchors_and_hrefs(self):
        ...
        for book_part in self.book_parts:
            ...
            for e in body.iter("*"):
                if "id" in e.attrib and not visible_elements_before(e):
                    ...
                    moved = immovable = False
                    for anchor, a_uri in self.anchor_uri.items():
                        if a_uri == uri:
                            if anchor not in self.immovable_anchors:
                                self.anchor_uri[anchor] = book_part.filename
                                moved = True
                                if self.DEBUG:
        ...
        resolve_toc_target(self.ncx_toc)
